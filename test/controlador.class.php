...
switch ($_POST['etapa'])
{
	...
	case 'seleccionar_concepto':
		require_once 'seleccionar_concepto.html';
		break;
	...
	case 'evaluacion_de_insidencia':
		if(!empty($_POST['empleado']))
			array_push($_SESSION['incidentes'], Modelo::buscar_empleado_por_id($_POST['empleado']));

		end($_SESSION['incidentes']);
		$empleados_capaces = Modelo::buscar_empleados_que_pueden_cubir_un_puesto(current($_SESSION['incidentes'])->puesto);
...
// 					var_dump($empleados_capaces[$key]);
		...
		require_once 'evaluacion_de_insidencia.php';
		break;
